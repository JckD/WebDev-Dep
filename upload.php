...
<?php
    ...
    if(isset($_POST["submit"])){
        ...
        if(mysqli_connect_errno()){
            array_push($results, "Failed to connect to MYSQL: ". mysqli_connect_errno());
        }
        else{
            ...
            if($check !== false) {
                ...
                array_push($results, "*File is not an image.");
                $uploadOk = 0;
            }

            // Check if file already exists
            if (file_exists($target_file)) {
                array_push($results, "*Sorry, file already exists.");
                $uploadOk = 0;
            }

            // Check file size
            if ($_FILES["fileToUpload"]["size"] > 500000) {
                array_push($results, "*Sorry, your file is too large.");
                $uploadOk = 0;
            }
            ...
            && $imageFileType != "gif" ) {
                array_push($results, "*Sorry, only JPG, JPEG, PNG & GIF files are allowed.");
                $uploadOk = 0;
            }

            // Check if $uploadOk is set to 0 by an error
            if ($uploadOk == 0) {
                array_push($results, "*Your file was not uploaded.");
            ...
            // if everything is ok, try to upload file
                ...
                if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_dir . $img_name . "." . $imageFileType)) {
                    ...
                    array_push($results, "The file ". basename( $_FILES["fileToUpload"]["name"]). " has been uploaded.");
                    
                    header("Location: editprofile.php");
                } else {
                    array_push($results, "*Sorry, there was an error uploading your file.");
                }
            }
        }
    }
